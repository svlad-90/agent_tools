...
1. Use `codex_tools/tools/cpp_light_code_map` as the first C/C++ orientation
   ...
   python -m codex_tools.tools.cpp_light_code_map symbols path/to/file.c --kind function
...
2. Move to `codex_tools/tools/cpp_code_map` once the build environment is
...
3. Run commands from the workspace root with:
   ...
   ```sh
   python -m codex_tools.tools.cpp_light_code_map <command> ...
...
7. Before reading or changing an existing C or C++ source file during early
   ...
   ```sh
   python -m codex_tools.tools.cpp_light_code_map diagnose path/to/file.cpp --json
   python -m codex_tools.tools.cpp_light_code_map outline path/to/file.cpp --compact
   ```
