...
WORKSPACE_ROOT = CODEX_TOOLS_ROOT.parent
...
def _single_task_bootstrap_markdown_files() -> tuple[Path, ...]:
    ...
    paths.extend(_relative_markdown_files(CODEX_TOOLS_ROOT / "rules"))
    paths.append(Path("codex_tools/knowledge/README.md"))
    paths.extend(_relative_markdown_files(CODEX_TOOLS_ROOT / "knowledge" / "topics"))
    paths.extend(
        sorted(
            path.relative_to(WORKSPACE_ROOT)
            for path in (CODEX_TOOLS_ROOT / "skills").glob("*/SKILL.md")
        )
    )

    for path in (
        Path("codex_tools/paf_workspace/templates/TASK_DESCRIPTION.md"),
        Path("codex_tools/paf_workspace/templates/TASK_CONTEXT.md"),
    ):
        if (WORKSPACE_ROOT / path).is_file():
            paths.append(path)
