---
name: cpp-code-map
description: Navigate, inspect, safely edit, and audit C/C++ source files with the
  workspace agent_tools.tools.cpp_code_map libclang-based tool. Use when the agent
  works on C or C++ code, needs exact symbol maps, guarded symbol/body edits, compile_commands.json
  based parsing, include insertion, batch edits, or DMA_Plantuml puml-audit checks
  in the real build environment.
---

<!-- GENERATED by agent_tools/tools/rules_sync -- DO NOT EDIT BY HAND. Source: agent_tools/skills/cpp-code-map/SKILL.md -->

# C++ Code Map

Use the workspace implementations at `agent_tools/tools/cpp_light_code_map` and
`agent_tools/tools/cpp_code_map`. Do not depend on a globally installed the agent
