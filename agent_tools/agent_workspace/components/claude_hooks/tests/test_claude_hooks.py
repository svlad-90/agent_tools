...
def test_claude_command_hook_uses_agent_tools_environment(
    tmp_path: Path,
    ...
    registry = ClaudeHookRegistry()
    seen: list[ClaudeHookRequest] = []
    registry.subscribe(ClaudeHookEvent.SESSION_START, lambda request: seen.append(request), subscriber_id="one")
