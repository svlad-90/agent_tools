...
from agent_tools.agent_workspace.components.claude_hooks.api import handle_command_hook as handle_claude_hook
...
from agent_tools.agent_workspace.components.codex_hooks.api import handle_command_hook as handle_codex_hook
from agent_tools.agent_workspace.components.harness_policy.api import HarnessStatusEvent
from agent_tools.agent_workspace.components.harness_policy.api import clear_harness_debug_events
from agent_tools.agent_workspace.components.harness_policy.api import clear_harness_status_subscriptions
from agent_tools.agent_workspace.components.harness_policy.api import load_harness_debug_events
from agent_tools.agent_workspace.components.harness_policy.api import register_claude_policy
from agent_tools.agent_workspace.components.harness_policy.api import register_codex_policy
from agent_tools.agent_workspace.components.harness_policy.api import subscribe_harness_status
from agent_tools.agent_workspace.components.harness_policy.src.commands import handle_claude_policy_hook
...
def test_harness_policy_emits_status_updates_for_codex_prompt(tmp_path: Path) -> None:
...
def test_harness_policy_blocks_codex_stop_after_work_without_journal_update(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
    registry = CodexHookRegistry()
    register_codex_policy(registry)
...
def test_harness_policy_allows_codex_stop_after_journal_update(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
    registry = CodexHookRegistry()
    register_codex_policy(registry)
...
def test_harness_policy_precompact_is_silent_when_current(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
...
def test_harness_policy_precompact_logs_pending_codex_checkpoint_when_journal_is_stale(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
...
def test_harness_policy_precompact_logs_pending_claude_checkpoint_when_journal_is_stale(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
    clear_harness_debug_events(tmp_path)
    registry = ClaudeHookRegistry()
    register_claude_policy(registry)
...
def test_harness_policy_postcompact_injects_current_task_slots(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
    registry = ClaudeHookRegistry()
    register_claude_policy(registry)
...
def test_harness_policy_codex_command_entrypoint_registers_default_policy(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)

    result = handle_codex_policy_hook(
        json.dumps({"hook_event_name": CodexHookEvent.USER_PROMPT_SUBMIT.value, "task_dir": str(task_dir), "session_id": "s1"})
    )
...
def test_harness_policy_claude_command_entrypoint_registers_default_policy(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)

    result = handle_claude_policy_hook(
...
def test_harness_policy_records_debug_events_with_tool_name(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
    clear_harness_debug_events(tmp_path)
    registry = CodexHookRegistry()
    register_codex_policy(registry)
        ...
        json.dumps(
            {
                ...
                "tool_input": {"command": "python3 -m pytest agent_tools/agent_workspace/components/harness_policy/tests"},
            }
        ),
        registry=registry,
    )
...
def test_harness_policy_records_context_injection_points(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
    clear_harness_debug_events(tmp_path)
    registry = CodexHookRegistry()
    register_codex_policy(registry)
...
def test_harness_policy_records_observed_hook_events(tmp_path: Path) -> None:
    task_dir = _task(tmp_path)
    registry = ClaudeHookRegistry()
    register_claude_policy(registry)
