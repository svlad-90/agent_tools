...
from ...ui_contract.api import set_ui_contract_metadata
from ...ui_contract.api import snapshot_widget_tree


def gtk_settings_ui_tree() -> UiTree:
    """Return the GTK-owned settings layout contract.

    This is a source-level snapshot of the GTK settings dialog. It deliberately
    avoids importing Gtk so contract tests can run in headless environments.
    """
    return UiTree(
        frontend="gtk",
        view="settings",
        ...
        nodes=(
            ...
            _spin("settings.text_font_size", "text_font_size", 8, 28, 1),
            _spin("settings.button_font_size", "button_font_size", 8, 28, 1),
            ...
            _spin("settings.limited_bash_head_tokens", "limited_bash_head_tokens", 100, 200_000, 100),
            _spin("settings.limited_bash_tail_tokens", "limited_bash_tail_tokens", 100, 200_000, 100),
            _spin("settings.limited_bash_heartbeat_seconds", "limited_bash_heartbeat_seconds", 1, 300, 1),
            _spin("settings.limited_bash_heartbeat_tokens", "limited_bash_heartbeat_tokens", 0, 200_000, 100),
            ...
            _spin("settings.dictionary_min_occurrences", "settings_dictionary_min_occurrences", 1, 20, 1),
            _spin("settings.dictionary_min_saving", "settings_dictionary_min_saving", 0, 10_000, 1),
            _spin("settings.dictionary_min_term_length", "settings_dictionary_min_term_length", 1, 200, 1),
            _spin("settings.dictionary_max_term_words", "settings_dictionary_max_term_words", 1, 20, 1),
            UiNode(
                "settings.dictionary_preview_text",
                "field",
            ...
            UiNode(
                "settings.mcp_group_validation",
                "field",
                label_key="settings_mcp_group_validation",
                widget="checkbox",
            ),
            UiNode(
                "settings.mcp_group_repo_guard",
                "field",
                label_key="settings_mcp_group_repo_guard",
                widget="checkbox",
            ),
            ...
            UiNode(
                "settings.mcp_group_paf_env",
                "field",
                label_key="settings_mcp_group_paf_env",
                widget="checkbox",
            ),
...
def mark_gtk_widget(
    ...
    *,
    ...
    set_name = getattr(widget, "set_name", None)
...
def snapshot_gtk_settings_runtime_tree(dialog: Any) -> UiTree:
    return snapshot_widget_tree(
        dialog,
        frontend="gtk",
        view="settings",
        root_id="settings.dialog",
...
def _spin(node_id: str, label_key: str, minimum: int, maximum: int, step: int) -> UiNode:
    return UiNode(
        node_id,
        "field",
        label_key=label_key,
