...
def test_actualize_task_reports_harness_policy_ready_for_existing_task(tmp_path: Path) -> None:
    ...
    assert _has_result(results, "PASS", "actualize-harness-policy-ready")
...
def test_actualize_cli_reports_harness_policy_ready(tmp_path: Path, capsys) -> None:
    task_dir = tmp_path / "tasks" / "sample-task"
    (task_dir / "front_door_bell.py").mkdir(parents=True)
    ...
    assert exit_code == 0
    assert "actualize-harness-policy-ready" in capsys.readouterr().out
