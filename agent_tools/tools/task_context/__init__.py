...
DICTIONARY_PREVIEW_TEXT = (
    "Agent Workspace renders TASK_CONTEXT.sqlite3 entries for active tasks. "
    "Agent Workspace keeps task context readable for humans and encoded for agents. "
    "Agent Workspace Details can show encoded task context with dictionary aliases. "
    "Agent Workspace Settings includes a Dictionary tab for tuning the task dictionary compiler.\n\n"
    "The task dictionary compiler reads summary and details fields from TASK_CONTEXT.sqlite3. "
    "The task dictionary compiler discovers repeated terms in summary and details, then decides whether an alias "
    ...
    "task_check validates TASK_CONTEXT.sqlite3 before push. task_check reports active journal size using rough "
    ...
    "inject_task_context_prompt controls whether Agent Workspace injects active task context into new and resumed "
    "agent prompts. inject_task_context_prompt defaults to true. Agent Workspace uses inject_task_context_prompt "
    "when preparing Codex and Claude launch prompts. Agent Workspace includes task_check failures in prompt context "
    "when task_check fails.\n\n"
    ...
    "Agent policy requires reading active task context before work. Agent policy requires resolving/staling "
    "superseded active entries. Agent policy requires adding stable domain terms through dictionary --add. "
    "Agent policy requires terse factual durable notes. Agent policy avoids turning task context into a changelog.\n\n"
    ...
    "aliases.\n\n"
    "TASK_CONTEXT.sqlite3 is the only task context source. TASK_CONTEXT.sqlite3 stores original summary, original "
    "details, encoded summary, encoded details, dictionary references, and append-only dictionary rows. "
    ...
    "journal compilation, not display rendering. Agent Workspace Dictionary settings include auto discovery, min "
