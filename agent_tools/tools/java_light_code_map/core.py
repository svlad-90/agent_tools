...
class JavaLightCodeMapError(Exception):
...
def compact_help() -> str:
    return "\n".join([
        "java_light_code_map help",
        "java_light_code_map map <java_file> [--compact] [--json]",
        "java_light_code_map outline <java_file> [--compact] [--json]",
        "java_light_code_map diagnose <java_file> [--json]",
        "java_light_code_map unmapped <java_file> [--json]",
        "java_light_code_map symbols <java_file> [--kind <kind>] [--name <text>] "
        "[--contains <line>] [--compact] [--json]",
        "java_light_code_map symbol-get <java_file> --symbol <name> [--with-doc] [--json]",
        "java_light_code_map imports <java_file> [--json]",
        "java_light_code_map annotations <java_file> [--json]",
        "java_light_code_map calls <java_file> [--symbol <name>] [--json]",
        "java_light_code_map call-graph <java_file> [--json]",
        "java_light_code_map refs <java_file> --name <identifier> [--scope <symbol>] [--json]",
        "java_light_code_map locals <java_file> --symbol <name> [--json]",
        "java_light_code_map complexity <java_file> [--symbol <name>] [--json]",
        "java_light_code_map parse-check <java_file> [--json]",
        "java_light_code_map index <java_file> [<java_file> ...] [--cache-dir <dir>] [--json]",
        "java_light_code_map index-dir <dir> [--include <glob>] [--exclude <glob>] "
        "[--cache-dir <dir>] [--json]",
        "java_light_code_map query --name <name> [--cache-dir <dir>] [--json]",
        "java_light_code_map rename-symbol <java_file> --symbol <name> --expect-hash <sha256> "
        "--new-name <identifier> [--scope <symbol>] [--check-only] [--json]",
        "java_light_code_map replace-symbol <java_file> --symbol <name> --expect-hash <sha256> "
        "(--replacement-env <VAR> | --replacement-file <path> | --replacement-text <text> | --replacement-stdin) "
        "[--check-only] [--json]",
        "java_light_code_map replace-symbol-body <java_file> --symbol <name> --expect-hash <sha256> "
        "(--replacement-env <VAR> | --replacement-file <path> | --replacement-text <text> | --replacement-stdin) "
        "[--check-only] [--json]",
        "java_light_code_map insert-before-symbol <java_file> --symbol <name> --expect-hash <sha256> "
        "(--snippet-env <VAR> | --snippet-file <path> | --snippet-text <text> | --snippet-stdin) "
        "[--check-only] [--json]",
        "java_light_code_map insert-after-symbol <java_file> --symbol <name> --expect-hash <sha256> "
        "(--snippet-env <VAR> | --snippet-file <path> | --snippet-text <text> | --snippet-stdin) "
        "[--check-only] [--json]",
    ])
...
def render_diagnose(file_path: Path, *, json_output: bool = False) -> str:
    ...
    packages_ok = True
    for package in ("tree-sitter", "tree-sitter-java"):
        try:
...
def render_imports(file_path: Path, *, json_output: bool = False) -> str:
    result = parse_light_file(file_path)
    source_bytes, tree = _tree_sitter_parse(result.source)
    imports = _collect_imports(tree.root_node, source_bytes, _line_start_offsets(result.source))
    if json_output:
        return json.dumps(_base_payload(file_path, engine=result.engine) | {
            "diagnostics": list(result.diagnostics),
            "imports": imports,
    ...
    for diagnostic in result.diagnostics:
        lines.append(f"diagnostic: {diagnostic}")
    lines.extend(f"{item['line']}: {item['text']}" for item in imports)
    return "\n".join(lines)


def render_annotations(file_path: Path, *, json_output: bool = False) -> str:
    result = parse_light_file(file_path)
    source_bytes, tree = _tree_sitter_parse(result.source)
    annotations = _collect_annotations(tree.root_node, source_bytes, _line_start_offsets(result.source))
    payload = _base_payload(file_path, engine=result.engine) | {"annotations": annotations}
    if json_output:
        return json.dumps(payload, indent=2)
    lines = [str(file_path), f"engine: {result.engine}", "semantic: false"]
    lines.extend(f"{annotation['line']}: {annotation['name']}" for annotation in annotations)
    return "\n".join(lines)
...
def render_index(file_paths: tuple[Path, ...],
                 *,
                 ...
                 json_output: bool = False) -> str:
    target_cache_dir = (cache_dir or Path(".cache/java_light_code_map")).resolve()
    ...
    if json_output:
        return json.dumps(payload, indent=2, sort_keys=True)
    lines = ["java_light_code_map :: index", f"ok: {str(ok).lower()}", f"cache_dir: {target_cache_dir}"]
...
def render_query(name: str, *, cache_dir: Path | None = None, json_output: bool = False) -> str:
    ...
    if json_output:
        return json.dumps(payload, indent=2, sort_keys=True)
    lines = ["java_light_code_map :: query", f"cache_dir: {target_cache_dir}", f"matches: {len(matches)}"]
...
def render_rename_symbol(file_path: Path,
    ...
    if not new_name.isidentifier():
        raise JavaLightCodeMapError("new name must be a valid identifier", details={"new_name": new_name})
    result = parse_light_file(file_path)
    symbol = _resolve_symbol(result.symbols, symbol_name, file_path)
    if symbol.hash != expected_hash:
        raise JavaLightCodeMapError("symbol hash mismatch",
                                   details={"file": str(file_path),
                                            "symbol": symbol.qualified_name,
                                            "expected_hash": expected_hash,
                                            "actual_hash": symbol.hash})
    old_name = symbol.name.rsplit(".", 1)[-1]
    source_bytes, tree = _tree_sitter_parse(result.source)
...
def render_replace_symbol_body(file_path: Path,
    ...
    if symbol.body_span is None or symbol.body_hash is None:
        raise JavaLightCodeMapError("symbol has no replaceable body",
                                   details={"file": str(file_path), "symbol": symbol.qualified_name})
    if symbol.body_hash != expected_hash:
        raise JavaLightCodeMapError("symbol body hash mismatch",
                                   details={
                                       "file": str(file_path),
                                       "symbol": symbol.qualified_name,
                                       "expected_hash": expected_hash,
...
def render_insert_relative_to_symbol(file_path: Path,
    ...
    symbol = _resolve_symbol(result.symbols, symbol_name, file_path)
    if symbol.hash != expected_hash:
        raise JavaLightCodeMapError("anchor symbol hash mismatch",
                                   details={"file": str(file_path),
                                            "symbol": symbol.qualified_name,
                                            "expected_hash": expected_hash,
                                            "actual_hash": symbol.hash})
...
def _tree_sitter_parse(source: str) -> tuple[bytes, Any]:
    try:
        ...
        raise JavaLightCodeMapError(
            "tree-sitter backend is required for java_light_code_map",
            details={"packages": ["tree-sitter", "tree-sitter-java"]},
        ) from exc
...
def _tree_sitter_symbols(node: Any,
    ...
    if node.type == "program":
        package_name = _root_package_name(node, source_bytes)
        child_parents = (package_name,) if package_name else parents
        for child in node.children:
            if child.type == "package_declaration":
                continue
            symbol = _tree_sitter_symbol(child, source, source_bytes, line_offsets, child_parents)
            if symbol is not None:
                symbols.append(symbol)
                continue
            symbols.extend(_tree_sitter_symbols(child, source, source_bytes, line_offsets, child_parents))
        return symbols
    for child in node.children:
...
def _collect_calls(node: Any,
    ...
    if node.type in {"method_invocation", "object_creation_expression"} and _node_in_span(node, target_span):
        callee = node.child_by_field_name("name")
        if callee is None and node.type == "object_creation_expression":
...
def _collect_refs(node: Any,
    ...
    if node.type in {"identifier", "field_identifier", "type_identifier"} and _node_in_span(node, target_span):
...
def _collect_annotations(node: Any, source_bytes: bytes, line_offsets: list[int]) -> list[dict[str, Any]]:
    annotations: list[dict[str, Any]] = []
    if node.type in {"annotation", "marker_annotation"}:
        span = _span_from_offsets(line_offsets, node.start_byte, node.end_byte)
        annotations.append({
            "kind": node.type,
            "name": _annotation_name(node, source_bytes),
            "line": span.start_line,
            "column": span.start_column,
    ...
    for child in node.children:
        annotations.extend(_collect_annotations(child, source_bytes, line_offsets))
    return annotations


def _annotation_name(node: Any, source_bytes: bytes) -> str:
    for child in node.children:
        if child.type in {"identifier", "type_identifier", "scoped_type_identifier"}:
            return _node_text(child, source_bytes)
    return _node_text(node, source_bytes).strip().split("(", 1)[0].lstrip("@")
...
def _collect_locals_into(node: Any,
    ...
    elif node.type in {"local_variable_declaration", "variable_declarator"}:
...
def _declaration_identifier_nodes(node: Any) -> list[Any]:
    ...
    for child in node.children:
        if child.type in {"identifier", "field_identifier"}:
            result.append(child)
        elif child.type in {"variable_declarator", "array_declarator"}:
...
def _complexity_walk(node: Any, source_bytes: bytes, counts: dict[str, Any], *, nesting: int) -> None:
    branch_types = {"if_statement", "ternary_expression", "switch_label"}
...
def _tree_sitter_symbol(node: Any,
    ...
    if not name:
        return None
    if kind in {"class", "interface", "enum", "record", "annotation_type"} and _tree_sitter_body_node(node) is None:
        return None
...
def _tree_sitter_kind(node: Any) -> str | None:
    return {
        ...
        "annotation_type_element_declaration": "annotation_member",
...
def _tree_sitter_name(node: Any, source_bytes: bytes) -> str | None:
    if node.type == "package_declaration":
        return _package_name(node, source_bytes)
    if node.type in {
        "class_declaration",
        ...
        return _field_name_text(node, source_bytes, "name")
    if node.type in {"field_declaration", "enum_constant"}:
...
def _tree_sitter_body_node(node: Any) -> Any | None:
    for child in node.children:
        if child.type in {"class_body", "interface_body", "enum_body", "constructor_body", "block"}:
            return child
    return None
...
def _declarator_name(node: Any, source_bytes: bytes) -> str | None:
    ...
    if node.type in {"scoped_identifier", "scoped_type_identifier", "generic_type"}:
        return _node_text(node, source_bytes).replace(" ", "")
    for child in node.children:
        if child.type in {"formal_parameters", "argument_list"}:
            break
...
def _field_name_text(node: Any, source_bytes: bytes, field_name: str) -> str | None:
...
def _package_name(node: Any, source_bytes: bytes) -> str:
    text = _node_text(node, source_bytes).strip()
    text = text.removeprefix("package").removesuffix(";").strip()
    return text or "default_package"


def _root_package_name(node: Any, source_bytes: bytes) -> str:
    for child in node.children:
        if child.type == "package_declaration":
            return _package_name(child, source_bytes)
    return ""
...
def _collect_imports(node: Any, source_bytes: bytes, line_offsets: list[int]) -> list[dict[str, Any]]:
    ...
    for child in node.children:
        imports.extend(_collect_imports(child, source_bytes, line_offsets))
    return imports
...
def _resolve_symbol(symbols: tuple[LightSymbol, ...], symbol_name: str, file_path: Path) -> LightSymbol:
    ...
    if not matches:
        raise JavaLightCodeMapError("symbol not found", details={"file": str(file_path), "symbol": symbol_name})
    if len(matches) > 1:
        raise JavaLightCodeMapError("symbol is ambiguous",
                                   details={"file": str(file_path),
                                            "symbol": symbol_name,
...
def _read_source(file_path: Path) -> str:
    try:
        ...
        raise JavaLightCodeMapError("failed to read source file",
