...
from .core import JavaLightCodeMapError
...
from .core import render_annotations
from .core import render_imports
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Map Java source structure without a build context.")
    subparsers = parser.add_subparsers(dest="command")

    map_parser = subparsers.add_parser("map", help="Print a structural Java symbol map.")
    ...
    imports_parser = subparsers.add_parser("imports", help="List package and import declarations.")
    _add_source_args(imports_parser)
    imports_parser.add_argument("--json", action="store_true")

    annotations_parser = subparsers.add_parser("annotations", help="List Java annotations.")
    _add_source_args(annotations_parser)
    ...
    index_dir_parser = subparsers.add_parser("index-dir", help="Index Java files under a directory.")
    ...
    try:
        if args.command in {"map", "outline"}:
            target = Path(args.java_file).resolve()
            ...
            target = Path(args.java_file).resolve()
            print(render_imports(target, json_output=args.json))
        elif args.command == "annotations":
            target = Path(args.java_file).resolve()
            print(render_annotations(target, json_output=args.json))
            ...
            print(render_index(tuple(Path(item).resolve() for item in args.java_files),
    ...
    except JavaLightCodeMapError as exc:
