...
_PAF_HIDE_TASK_ENV_VAR = "PAF_HIDE_TASK_ENV"
_TASK_ACTION_LOGS_DIR = Path("report") / "logs"
...
def task_action_shell_command(action: TaskAction) -> str:
    ...
    env_values[_PAF_HIDE_TASK_ENV_VAR] = "1"
    env = " ".join(
        f"{key}={shlex.quote(value)}"
        for key, value in sorted(env_values.items())
    )
    prefix = f"{env} " if env else ""
    log_name = _task_action_log_basename(action.action_id)
    inner = "\n".join(
        [
            "set -o pipefail",
            ...
            "    __agent_task_dir=$PWD",
            "fi",
            f"__agent_log_dir=\"$__agent_task_dir/{_TASK_ACTION_LOGS_DIR.as_posix()}\"",
...
def task_action_windows_command(action: TaskAction) -> str:
    ...
    env_values[_PAF_HIDE_TASK_ENV_VAR] = "1"
    log_name = _task_action_log_basename(action.action_id)
    log_dir = action.cwd / _TASK_ACTION_LOGS_DIR
...
def _task_action_log_basename(action_id: str) -> str:
