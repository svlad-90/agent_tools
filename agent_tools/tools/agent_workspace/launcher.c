...
static void show_error(const char *message) {
    pid_t pid = fork();
    if (pid == 0) {
        execlp("zenity", "zenity", "--error", "--title=Agent Workspace", "--text", message, NULL);
        execlp("xmessage", "xmessage", "-title", "Agent Workspace", message, NULL);
        _exit(127);
    }
    if (pid > 0) {
        int status = 0;
...
static int run_python(const char *python, int argc, char **argv) {
    ...
    args[0] = (char *)python;
    args[1] = "-m";
    args[2] = "agent_tools.tools.agent_workspace";
...
int main(int argc, char **argv) {
    ...
    if (workspace_dir(workspace, sizeof(workspace)) != 0) {
        show_error("Could not resolve the agent-workspace location.");
        return 1;
    }
    ...
    show_error("Agent Workspace could not start: python3/python was not found.");
    return status;
}
