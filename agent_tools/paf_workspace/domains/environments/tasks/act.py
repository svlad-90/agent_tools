...
class ensure_agent_tools_act_image(EnvironmentTask):
    """Ensure the Agent tools act driver image is available."""

    def __init__(self):
        super().__init__()
        self.set_name(ensure_agent_tools_act_image.__name__)
...
class check_agent_tools_act_image(EnvironmentTask):
    """Check the Agent tools act driver image without building it."""

    def __init__(self):
        super().__init__()
        self.set_name(check_agent_tools_act_image.__name__)

    def execute(self):
        self.check_image_alias(self.environment_string("agent_tools_act", "image", "agent-tools-act"))


class check_agent_tools_act_tools(EnvironmentTask):
    """Run Agent tools act driver smoke checks inside its Docker image."""

    def __init__(self):
        super().__init__()
        self.set_name(check_agent_tools_act_tools.__name__)

    def execute(self):
        if self.bool_param("SKIP_AGENT_TOOLS_ACT_TOOLS_CHECK"):
            logger.info("Skip Agent tools act tools check: SKIP_AGENT_TOOLS_ACT_TOOLS_CHECK is enabled")
            return
        self.docker_tools_check(
            self.environment_string("agent_tools_act", "container", "agent-tools-act-workspace"),
            runtime.AGENT_TOOLS_ACT_TOOL_CHECK_COMMAND,
            "AGENT_TOOLS_ACT_TOOLS_CHECK_TIMEOUT_SEC",
        )


class validate_agent_tools_act(EnvironmentTask):
    """Run the Agent tools GitHub Actions workflow through act."""

    def __init__(self):
        super().__init__()
        self.set_name(validate_agent_tools_act.__name__)

    def execute(self):
        run = runtime.AgentToolsActRun(
            workflow=self.param(
                "AGENT_TOOLS_ACT_WORKFLOW",
                self.environment_string("agent_tools_act", "workflow", ".github/workflows/diff-report.yml"),
            ),
            runner_image=self.param(
                "AGENT_TOOLS_ACT_RUNNER_IMAGE",
                self.environment_string("agent_tools_act", "runner_image", "ghcr.io/catthehacker/ubuntu:act-latest"),
            ),
            extra_args=self.lines_param("AGENT_TOOLS_ACT_ARGS"),
        )
        self.docker_subprocess_must_succeed(
            self.environment_string("agent_tools_act", "container", "agent-tools-act-workspace"),
            runtime.agent_tools_act_validate_command(run),
            timeout=int(self.param("AGENT_TOOLS_ACT_TIMEOUT_SEC", "0") or "0"),
