"""Structural checks for agent_tools and the workspace PAF layout."""
...
REQUIRED_AGENT_TOOLS_DIRS = ("knowledge", "paf_workspace", "rules", "skills", "tools")
...
def check_agent_tools_structure(root: Path) -> list[StructureIssue]:
    """Return structural issues for the agent_tools workspace layout."""

    root = root.resolve()
    issues: list[StructureIssue] = []

    for dirname in REQUIRED_AGENT_TOOLS_DIRS:
        if not (root / dirname).is_dir():
            issues.append(StructureIssue(root / dirname, "missing required agent_tools directory"))

    for dirname in FORBIDDEN_AGENT_TOOLS_DIRS:
        if (root / dirname).exists():
            issues.append(
                StructureIssue(root / dirname, "legacy agent_tools path must move into the owning namespace")
            )

    for path in sorted(root.iterdir()):
        if not path.is_dir() or path.name.startswith("."):
            continue
        if path.name not in ALLOWED_AGENT_TOOLS_DIRS:
            issues.append(StructureIssue(path, "unexpected top-level agent_tools directory"))
...
def assert_agent_tools_structure(root: Path) -> None:
    issues = check_agent_tools_structure(root)
    if issues:
        formatted = "\n".join(issue.format(root) for issue in issues)
        raise AssertionError(f"agent_tools structure check failed:\n{formatted}")
