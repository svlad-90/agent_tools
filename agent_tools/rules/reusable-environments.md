...
3. The Dockerfile is the source of truth for Docker-based environments. Do not
   ...
   `agent_tools/paf_workspace/domains/environments/domain.yaml` must declare
   ...
   - importable `tree_sitter` and `tree_sitter_cpp` Python packages for
     `python3 -m agent_tools.tools.cpp_light_code_map`;
   ...
   `python3-clang` so
   `python3 -m agent_tools.tools.cpp_code_map doctor|map|parse-check` can run
   ...
   `agent_tools/paf_workspace/domains/environments/lib/capabilities.py` and let
...
4. Store reusable workspace environment automation under
   `agent_tools/paf_workspace/domains/environments/`. Each environment should
   ...
   under `lib/`.
   Historical `agent_tools/environments/` directories are legacy locations and
...
9. For tasks that need a reusable build or runtime environment, first look for
   ...
   `agent_tools/paf_workspace/domains/environments/`. If none fits, extend the
