---
name: cpp-code-map
description: Navigate, inspect, safely edit, and audit C/C++ source files with the workspace agent_tools.tools.cpp_code_map libclang-based tool. Use when Codex works on C or C++ code, needs exact symbol maps, guarded symbol/body edits, compile_commands.json based parsing, include insertion, batch edits, or DMA_Plantuml puml-audit checks in the real build environment.
rule: agent_tools/rules/cpp-code.md
---
...
python -m agent_tools.tools.cpp_light_code_map symbols path/to/file.c --kind function --json
```
...
```sh
python -m agent_tools.tools.cpp_code_map map path/to/file.c --compile-db path/to/build
```
...
```sh
python -m agent_tools.tools.cpp_code_map map path/to/file.c --compile-db build
```
...
```sh
python -m agent_tools.tools.cpp_code_map doctor path/to/file.c --compile-db build --json
```
...
```sh
python -m agent_tools.tools.cpp_code_map symbol-get path/to/file.c \
...
```sh
python -m agent_tools.tools.cpp_code_map replace-symbol-body path/to/file.c \
...
```sh
python -m agent_tools.tools.cpp_code_map parse-check path/to/file.c --compile-db build
```
...
agent_tools/paf_workspace/run-paf.sh \
  ...
  check-only \
  --yaml-config agent_tools/paf_workspace/domains/environments/profiles/cpp-code-map-zephyr-xen.yaml \
...
container mount, checks that `clang.cindex` and `agent_tools` import inside the
...
```sh
python -m agent_tools.tools.cpp_light_code_map map path/to/file.c
python -m agent_tools.tools.cpp_light_code_map symbol-get path/to/file.c --symbol name --json
```
...
python -m agent_tools.tools.cpp_light_code_map outline path/to/file.c --compact
python -m agent_tools.tools.cpp_light_code_map diagnose path/to/file.c --json
python -m agent_tools.tools.cpp_light_code_map unmapped path/to/file.c --json
python -m agent_tools.tools.cpp_light_code_map symbols path/to/file.c --kind function --name init
python -m agent_tools.tools.cpp_light_code_map includes path/to/file.c --json
python -m agent_tools.tools.cpp_light_code_map macros path/to/file.c --json
python -m agent_tools.tools.cpp_light_code_map calls path/to/file.c --symbol function_name --json
python -m agent_tools.tools.cpp_light_code_map call-graph path/to/file.c --json
python -m agent_tools.tools.cpp_light_code_map refs path/to/file.c --name identifier --scope function_name --json
python -m agent_tools.tools.cpp_light_code_map locals path/to/file.c --symbol function_name --json
python -m agent_tools.tools.cpp_light_code_map complexity path/to/file.c --symbol function_name --json
```

For repeated workspace navigation, cache structural symbol maps and query them:

```sh
python -m agent_tools.tools.cpp_light_code_map index src/a.c src/b.cpp --cache-dir /tmp/cpp-light-index
python -m agent_tools.tools.cpp_light_code_map index-dir src --include '*.c' --include '*.cpp' --cache-dir /tmp/cpp-light-index
python -m agent_tools.tools.cpp_light_code_map query --name symbol --cache-dir /tmp/cpp-light-index
```
...
```sh
python -m agent_tools.tools.cpp_light_code_map replace-symbol-body path/to/file.c \
  --symbol function_name --expect-hash <body_hash> --replacement-text $'\n\treturn 0;\n' --check-only
python -m agent_tools.tools.cpp_light_code_map rename-symbol path/to/file.c \
...
```sh
python -m agent_tools.tools.cpp_code_map puml-audit path/to/file.cpp --compile-db build
```
