...
`workspace-policy.yaml` defines workspace-wide checks. `repos/*.yaml` and
`repos/<repo>/*.yaml` add checks and identity evidence for specific
repositories. Task-local policy may be added as `tasks/<task>/TASK_GUARD.yaml`.
...
2. `repos/*.yaml` or `repos/<repo>/*.yaml` for repository identity evidence
...
`verify_command`. Repository-specific Python checks can live next to the
repository YAML and use `backend: python`, `module: <file.py>`, and
`function: <callable>`.
